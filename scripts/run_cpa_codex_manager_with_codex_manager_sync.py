"""
Run CPA-Codex-Manager with an external Codex-Manager sync sidecar.

This script does not patch CPA-Codex-Manager source behavior. It:
1. Starts `webui.py` with env-based port defaults (`8000`)
...
def ensure_project_python() -> None:
    ...
    if os.environ.get("CPA_CODEX_MANAGER_BOOTSTRAPPED") == "1":
        return

    env = os.environ.copy()
    env["CPA_CODEX_MANAGER_BOOTSTRAPPED"] = "1"
    os.execve(
        str(target_python),
...
def resolve_console_env() -> Dict[str, str]:
    env = os.environ.copy()
    port = str(
        env.get("CPA_CODEX_MANAGER_PORT")
        or env.get("APP_PORT")
...
def start_webui(env: Dict[str, str]) -> subprocess.Popen:
    ...
    log(f"starting CPA-Codex-Manager on port {env['APP_PORT']}")
...
def main() -> int:
    ...
    start_web = (
        str(os.environ.get("CPA_CODEX_MANAGER_START_WEBUI") or "true").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    try:
        ...
        log("CPA_CODEX_MANAGER_START_WEBUI=false; sync sidecar running without launching webui")
        while True:
            time.sleep(1)
